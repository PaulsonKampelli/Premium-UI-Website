...
const Slide05Luxury = ({ isActive, onInquire }) => {
  ...
  return (
    <SlideWrapper isActive={isActive} className="items-center text-center">
      ...
      <div className="flex flex-col md:flex-row gap-12 items-center max-w-6xl mx-auto mb-20">
        <motion.div 
          initial={{ opacity: 0, scale: 0.95 }}
          animate={isActive ? { opacity: 1, scale: 1 } : {}}
          transition={{ delay: 0.4 }}
          className="md:w-1/2 aspect-video bg-bg-card border border-gold overflow-hidden"
        >
          <img 
        ...
        <div className="md:w-1/2 text-left space-y-8">
          <div className="flex flex-wrap gap-4">
            {brands.map((brand, i) => (
              <motion.div
                ...
                className="border border-gold/30 px-6 py-2 text-white text-[10px] font-bold uppercase tracking-[0.4em] bg-white/5"
              >
                {brand}
              </motion.div>
            ))}
          ...
          <div className="grid grid-cols-1 gap-8">
            ...
            >
              <h4 className="text-white font-bold uppercase tracking-widest text-xs mb-4 text-gold">Premium Positioning</h4>
              <p className="text-gray-text text-sm leading-relaxed mb-4">
                Our luxury corridor is a curated ecosystem where brand equity is protected by strict category exclusivity and elevated visual standards.
              </p>
              <ul className="space-y-2 text-gray-text text-sm leading-relaxed">
