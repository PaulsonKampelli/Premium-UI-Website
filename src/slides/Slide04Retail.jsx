...
const Slide04Retail = ({ isActive, onInquire }) => {
  ...
  return (
    <SlideWrapper isActive={isActive}>
      ...
      <h2 className="headline-lg font-display text-white mb-6">450+ Brands. Room for Yours.</h2>

      <div className="flex md:grid md:grid-cols-3 gap-4 md:gap-4 mb-6 md:mb-8 overflow-x-auto pb-4 snap-x custom-scrollbar w-full">
        {categories.map((cat, i) => (
          <motion.div
            ...
            className="bg-bg-card border border-border-color p-6 md:p-5 flex flex-col items-start h-full group hover:border-gold transition-colors cursor-pointer min-w-[280px] md:min-w-0 shrink-0 snap-center"
